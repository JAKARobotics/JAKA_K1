...
#include "jaka_k1_planner/JAKAZuRobot.h"
#include "jaka_k1_planner/jkerr.h"
#include "jaka_k1_planner/jktypes.h"
...
static IndexMapDual build_index_map_dual(const vector<string>& names)
{
  ...
  const char* Lnames[7] = {"rb1_j1","rb1_j2","rb1_j3","rb1_j4","rb1_j5","rb1_j6","rb1_j7"};
  const char* Rnames[7] = {"rb2_j1","rb2_j2","rb2_j3","rb2_j4","rb2_j5","rb2_j6","rb2_j7"};
...
static bool build_index_map_single(const vector<string>& names,
  ...
  for (int i=0;i<(int)names.size();++i) idx[names[i]] = i;
  for (int i=0;i<7;i++) {
    string jn = string(prefix) + to_string(i+1); // e.g. "rb1_j3"
...
static inline unsigned int steps_from_dt(double dt_sec, double servo_period=0.008)
{
...
void execute_goal(const shared_ptr<GoalHandle> gh, rclcpp::Node::SharedPtr node, int kind)
{
  ...
  if (kind==-1) {
    ...
    const char* pref = (kind==0) ? "rb1_j" : "rb2_j";
  ...
  if (kind==-1) {
    ...
    for (size_t i=0; i<traj.points.size(); ++i)
    {
      ...
      last_t = t;
      const unsigned int step = steps_from_dt(dt, 0.008);
      ...
      next_time += chrono::milliseconds(8);
    ...
    // compute deadline from last point time + margin (safer than fixed 8s)
  ...
  // Single-arm (left-arm only or right-arm only)
    ...
    for (size_t i=0; i<traj.points.size(); ++i)
    {
      ...
      // keep cadence close to servo period (8 ms) to avoid flooding
      next_time += chrono::milliseconds(8);
      this_thread::sleep_until(next_time);
    }
...
void joint_states_callback(rclcpp::Publisher<sensor_msgs::msg::JointState>::SharedPtr &joint_states_pub)
{
    ...
    for (int i=0;i<7;i++) {
        ...
        joint_msg.name.push_back("rb1_j" + to_string(i+1));
    }
    for (int i=0;i<7;i++) {
        joint_msg.position.push_back(okR ? jr.jVal[i] : numeric_limits<double>::quiet_NaN());
        joint_msg.name.push_back("rb2_j" + to_string(i+1));
    }
...
int main(int argc, char** argv)
{
    ...
    auto node = rclcpp::Node::make_shared("k1_moveit_server");
    ...
    auto dual_server = rclcpp_action::create_server<Follow>(
        node,
        "/jaka_k1_full_robot_controller/follow_joint_trajectory",
        // goal callback
        [](const rclcpp_action::GoalUUID&,
        shared_ptr<const Follow::Goal>) {
            RCLCPP_INFO(rclcpp::get_logger("k1_moveit_server"), "Received full_robot goal request");
            // (void)goal; // Avoid unused parameter warning
            return rclcpp_action::GoalResponse::ACCEPT_AND_EXECUTE;
        },
        // cancel callback
        [](shared_ptr<GoalHandle>) {
            RCLCPP_INFO(rclcpp::get_logger("k1_moveit_server"), "Received full_robot cancel request");
            return rclcpp_action::CancelResponse::ACCEPT;
        },
        // execute callback
        [node](shared_ptr<GoalHandle> gh) {
            RCLCPP_INFO(rclcpp::get_logger("k1_moveit_server"), "Executing full_robot goal");
    ...
    auto left_server = rclcpp_action::create_server<Follow>(
        node,
        "/jaka_k1_arm_l_controller/follow_joint_trajectory",
        // goal callback
        [](const rclcpp_action::GoalUUID&,
        shared_ptr<const Follow::Goal>) {
            RCLCPP_INFO(rclcpp::get_logger("k1_moveit_server"), "Received arm_l goal request");
            // (void)goal; // Avoid unused parameter warning
            return rclcpp_action::GoalResponse::ACCEPT_AND_EXECUTE;
        },
        // cancel callback
        [](shared_ptr<GoalHandle>) {
            RCLCPP_INFO(rclcpp::get_logger("k1_moveit_server"), "Received arm_l cancel request");
            return rclcpp_action::CancelResponse::ACCEPT;
        },
        // execute callback
        [node](shared_ptr<GoalHandle> gh) {
            RCLCPP_INFO(rclcpp::get_logger("k1_moveit_server"), "Executing arm_l goal");
    ...
    auto right_server = rclcpp_action::create_server<Follow>(
        node,
        "/jaka_k1_arm_r_controller/follow_joint_trajectory",
        // goal callback
        [](const rclcpp_action::GoalUUID&,
        shared_ptr<const Follow::Goal>) {
            RCLCPP_INFO(rclcpp::get_logger("k1_moveit_server"), "Received arm_r goal request");
            // (void)goal; // Avoid unused parameter warning
            return rclcpp_action::GoalResponse::ACCEPT_AND_EXECUTE;
        },
        // cancel callback
        [](shared_ptr<GoalHandle>) {
            RCLCPP_INFO(rclcpp::get_logger("k1_moveit_server"), "Received arm_r cancel request");
            return rclcpp_action::CancelResponse::ACCEPT;
        },
        // execute callback
        [node](shared_ptr<GoalHandle> gh) {
            RCLCPP_INFO(rclcpp::get_logger("k1_moveit_server"), "Executing arm_r goal");
            execute_goal(gh, node, 1);
        }
    );

    RCLCPP_INFO(rclcpp::get_logger("k1_moveit_server"), "==================K1 Moveit Start==================");

...
#include "jaka_k1_msgs/msg/robot_states.hpp"
#include "jaka_k1_msgs/srv/move.hpp"  
#include "jaka_k1_msgs/srv/servo_move_enable.hpp"  
#include "jaka_k1_msgs/srv/servo_move.hpp"
#include "jaka_k1_msgs/srv/get_default_base.hpp" 
#include "jaka_k1_msgs/srv/set_tool_offset.hpp" 
#include "jaka_k1_msgs/srv/get_tool_offset.hpp" 
#include "jaka_k1_msgs/srv/set_tool_payload.hpp" 
#include "jaka_k1_msgs/srv/get_tool_payload.hpp" 
#include "jaka_k1_msgs/srv/drag_mode.hpp" 
#include "jaka_k1_msgs/srv/set_collision_level.hpp" 
#include "jaka_k1_msgs/srv/get_collision_level.hpp" 
#include "jaka_k1_msgs/srv/get_fk.hpp" 
#include "jaka_k1_msgs/srv/get_ik.hpp" 
#include "jaka_k1_msgs/srv/clear_error.hpp"
#include "jaka_k1_msgs/srv/get_sdk_version.hpp"
#include "jaka_k1_msgs/srv/set_debug_mode.hpp"   
#include "jaka_k1_msgs/srv/get_dh_params.hpp"

#include "jaka_k1_driver/JAKAZuRobot.h"
#include "jaka_k1_driver/jkerr.h"
#include "jaka_k1_driver/jktypes.h"
...
bool linear_move_callback(
    ...
    shared_ptr<jaka_k1_msgs::srv::Move::Response> res)
{
...
bool servo_move_enable_callback(
    const shared_ptr<jaka_k1_msgs::srv::ServoMoveEnable::Request> req,
    shared_ptr<jaka_k1_msgs::srv::ServoMoveEnable::Response> res)
{
...
bool edg_servo_p_callback(
    const shared_ptr<jaka_k1_msgs::srv::ServoMove::Request> req,
    shared_ptr<jaka_k1_msgs::srv::ServoMove::Response> res)
{
...
bool get_default_base_callback(
    const shared_ptr<jaka_k1_msgs::srv::GetDefaultBase::Request> req,
    shared_ptr<jaka_k1_msgs::srv::GetDefaultBase::Response> res)
{
...
bool set_tool_offset_callback(
    const shared_ptr<jaka_k1_msgs::srv::SetToolOffset::Request> req,
    shared_ptr<jaka_k1_msgs::srv::SetToolOffset::Response> res)
{
...
bool get_tool_offset_callback(
    const shared_ptr<jaka_k1_msgs::srv::GetToolOffset::Request> req,
    shared_ptr<jaka_k1_msgs::srv::GetToolOffset::Response> res)
{
    // Validate robot_index
    if (req->robot_index != 0 && req->robot_index != 1) {
        res->ret = -1;
...
bool set_tool_payload_callback(
    const shared_ptr<jaka_k1_msgs::srv::SetToolPayload::Request> req,
    shared_ptr<jaka_k1_msgs::srv::SetToolPayload::Response> res)
{
...
bool get_tool_payload_callback(
    const shared_ptr<jaka_k1_msgs::srv::GetToolPayload::Request> req,
    shared_ptr<jaka_k1_msgs::srv::GetToolPayload::Response> res)
{
    // robot_index: only per-arm (no -1)
    if (req->robot_index != 0 && req->robot_index != 1) {
        res->ret = -1;
...
bool drag_mode_callback(
    const shared_ptr<jaka_k1_msgs::srv::DragMode::Request> req,
    shared_ptr<jaka_k1_msgs::srv::DragMode::Response> res)
{
    if (req->robot_index != -1 && req->robot_index != 0 && req->robot_index != 1) {
        res->ret = -1;
        res->message = "robot_index must be -1 (DUAL), 0 (LEFT), or 1 (RIGHT)";
...
bool set_collision_level_callback(
    const shared_ptr<jaka_k1_msgs::srv::SetCollisionLevel::Request> req,
    shared_ptr<jaka_k1_msgs::srv::SetCollisionLevel::Response> res)
{
...
bool get_collision_level_callback(
    const shared_ptr<jaka_k1_msgs::srv::GetCollisionLevel::Request> req,
    shared_ptr<jaka_k1_msgs::srv::GetCollisionLevel::Response> res)
{
    // robot_index is per-arm only (no -1)
    if (req->robot_index != 0 && req->robot_index != 1) {
        res->ret = -1;
...
bool get_fk_callback(
    const shared_ptr<jaka_k1_msgs::srv::GetFK::Request> req,
    shared_ptr<jaka_k1_msgs::srv::GetFK::Response> res)
{
...
bool get_ik_callback(
    const shared_ptr<jaka_k1_msgs::srv::GetIK::Request> req,
    shared_ptr<jaka_k1_msgs::srv::GetIK::Response> res)
{
...
bool clear_error_callback(
    const shared_ptr<jaka_k1_msgs::srv::ClearError::Request> /*request*/,
    shared_ptr<jaka_k1_msgs::srv::ClearError::Response> res)
{
...
bool get_sdk_version_callback(
    const shared_ptr<jaka_k1_msgs::srv::GetSdkVersion::Request> /*request*/,
    shared_ptr<jaka_k1_msgs::srv::GetSdkVersion::Response> res)
{
...
bool set_debug_mode_callback(
    const shared_ptr<jaka_k1_msgs::srv::SetDebugMode::Request> req,
    shared_ptr<jaka_k1_msgs::srv::SetDebugMode::Response> res)
{
...
bool get_dh_params_callback(
    const shared_ptr<jaka_k1_msgs::srv::GetDHParams::Request> req,
    shared_ptr<jaka_k1_msgs::srv::GetDHParams::Response> res)
{
...
void tool_position_callback_edg(
    const rclcpp::Publisher<geometry_msgs::msg::TwistStamped>::SharedPtr& tool_position_pub)
{
    ...
    {
        RCLCPP_ERROR(rclcpp::get_logger("tool_position_callback_edg"), "TCP publisher is not initialized!");
        return;
    }
    ...
    for (int robot_index = 0; robot_index < 2; ++robot_index) {
        ...
        if (ret != 0) 
        { 
            RCLCPP_ERROR(rclcpp::get_logger("tool_position_callback_edg"), "edg_get_stat failed: %s", mapErr[ret].c_str()); 
            return; 
        }
...
// Joint state via EDG (per arm)
void joint_position_callback_edg(
    const rclcpp::Publisher<sensor_msgs::msg::JointState>::SharedPtr& joint_position_pub)
{
    // Check if publisher is valid
    if (!joint_position_pub) 
    { 
        RCLCPP_ERROR(rclcpp::get_logger("joint_position_callback_edg"), "Joint publisher not initialized"); 
        return; 
    }
    ...
    for (int robot_index = 0; robot_index < 2; ++robot_index) {
        ...
        if (ret != 0) 
        { 
            RCLCPP_ERROR(rclcpp::get_logger("joint_position_callback_edg"), "edg_get_stat failed: %s", mapErr[ret].c_str()); 
            continue; 
        }
...
void robot_states_callback(const rclcpp::Publisher<jaka_k1_msgs::msg::RobotStates>::SharedPtr& robot_states_pub)
{
    if (!robot_states_pub) {
    ...
    jaka_k1_msgs::msg::RobotStates robot_states{};
    bool ok = true;
    ...
    while (rclcpp::ok())
    {
        ...
        else 
        {
            ...
            else 
            {
                ...
                tool_position_callback_edg(tool_position_pub);
                joint_position_callback_edg(joint_position_pub);
            }
...
int main(int argc, char *argv[])
{
    ...
    auto node = rclcpp::Node::make_shared("jaka_k1_driver");
    ...
    //--------Services------------//
    auto linear_move_service = node->create_service<jaka_k1_msgs::srv::Move>("/jaka_k1_driver/linear_move", &linear_move_callback);
    auto joint_move_service = node->create_service<jaka_k1_msgs::srv::Move>("/jaka_k1_driver/joint_move", &joint_move_callback);
    auto servo_move_enable_service = node->create_service<jaka_k1_msgs::srv::ServoMoveEnable>("/jaka_k1_driver/servo_move_enable", &servo_move_enable_callback);
    auto edg_servo_p_service = node->create_service<jaka_k1_msgs::srv::ServoMove>("/jaka_k1_driver/edg_servo_p", &edg_servo_p_callback);
    auto edg_servo_j_service = node->create_service<jaka_k1_msgs::srv::ServoMove>("/jaka_k1_driver/edg_servo_j", &edg_servo_j_callback);
    auto stop_move_service = node->create_service<std_srvs::srv::Empty>("/jaka_k1_driver/stop_move", &stop_move_callback);
    auto get_default_base_service = node->create_service<jaka_k1_msgs::srv::GetDefaultBase>("/jaka_k1_driver/get_default_base", &get_default_base_callback);
    auto set_tool_offset_service = node->create_service<jaka_k1_msgs::srv::SetToolOffset>("/jaka_k1_driver/set_tool_offset", &set_tool_offset_callback);
    auto get_tool_offset_service = node->create_service<jaka_k1_msgs::srv::GetToolOffset>("/jaka_k1_driver/get_tool_offset", &get_tool_offset_callback);
    auto set_tool_payload_service = node->create_service<jaka_k1_msgs::srv::SetToolPayload>("/jaka_k1_driver/set_tool_payload", &set_tool_payload_callback);
    auto get_tool_payload_service = node->create_service<jaka_k1_msgs::srv::GetToolPayload>("/jaka_k1_driver/get_tool_payload", &get_tool_payload_callback);
    auto drag_move_service = node->create_service<jaka_k1_msgs::srv::DragMode>("/jaka_k1_driver/drag_mode", &drag_mode_callback);
    auto set_collision_level_service = node->create_service<jaka_k1_msgs::srv::SetCollisionLevel>("/jaka_k1_driver/set_collision_level", &set_collision_level_callback);
    auto get_collision_level_service = node->create_service<jaka_k1_msgs::srv::GetCollisionLevel>("/jaka_k1_driver/get_collision_level", &get_collision_level_callback);
    auto get_fk_service = node->create_service<jaka_k1_msgs::srv::GetFK>("/jaka_k1_driver/get_fk", &get_fk_callback);
    auto get_ik_service = node->create_service<jaka_k1_msgs::srv::GetIK>("/jaka_k1_driver/get_ik", &get_ik_callback);
    auto clear_error_service = node->create_service<jaka_k1_msgs::srv::ClearError>("/jaka_k1_driver/clear_error", &clear_error_callback);
    auto get_sdk_version_service = node->create_service<jaka_k1_msgs::srv::GetSdkVersion>("/jaka_k1_driver/get_sdk_version", &get_sdk_version_callback);
    auto set_debug_mode_service = node->create_service<jaka_k1_msgs::srv::SetDebugMode>("/jaka_k1_driver/set_debug_mode", &set_debug_mode_callback);
    auto get_dh_params_service = node->create_service<jaka_k1_msgs::srv::GetDHParams>("/jaka_k1_driver/get_dh_params", &get_dh_params_callback);

    // //3.1 End position pose status information reporting
    tool_position_pub = node->create_publisher<geometry_msgs::msg::TwistStamped>("/jaka_k1_driver/tool_position", 10);
    // //3.2 Joint status information reporting
    joint_position_pub = node->create_publisher<sensor_msgs::msg::JointState>("/jaka_k1_driver/joint_position", 10);
    // //3.3 Report robot event status information
    robot_states_pub = node->create_publisher<jaka_k1_msgs::msg::RobotStates>("/jaka_k1_driver/robot_states", 10);
